# ValutaTrade Hub

Виртуальная торговая платформа для управления мультивалютным портфелем с поддержкой крипто и фиатных валют.

## 📋 Возможности

- ✅ **Управление пользователями**: регистрация, аутентификация, безопасное хранение паролей
- ✅ **Мультивалютный портфель**: хранение крипто и фиатных валют
- ✅ **Торговые операции**: покупка и продажа валют по актуальным курсам
- ✅ **Parser Service**: автоматическое обновление курсов из внешних API
- ✅ **Логирование**: детальное отслеживание всех операций
- ✅ **Обработка ошибок**: понятные сообщения об ошибках

## 🚀 Установка и запуск

```bash
# 1. Клонировать репозиторий
git clone <ваш-репозиторий>
cd finalproject_<фамилия>_<группа>

# 2. Установить зависимости
make install

# 3. Запустить приложение
make project
# Или напрямую:
poetry run project
```

## Основные команды

### Управление пользователями

```bash
# Регистрация нового пользователя
register --username alice --password 1234

# Вход в систему
login --username alice --password 1234

# Выход
logout

# Информация о текущем пользователе
whoami
```

### Работа с портфелем

```bash
# Показать портфель (в USD)
showportfolio

# Показать портфель в другой валюте
showportfolio --base EUR

# Купить валюту
buy --currency BTC --amount 0.05

# Продать валюту
sell --currency BTC --amount 0.01
```

### Курсы валют

```bash
# Получить курс между валютами
getrate --from USD --to BTC

# Показать все курсы из кеша
show-rates

# Показать топ криптовалют
show-rates --top 3

# Показать курс конкретной валюты
show-rates --currency BTC

# Показать курсы в другой базовой валюте
show-rates --base EUR
```

### Пример выполнения команд проекта (часть 1)

[![asciicast](https://asciinema.org/a/M5lZrhgeIuj8aGkv9wobJVGFZ.svg)](https://asciinema.org/a/M5lZrhgeIuj8aGkv9wobJVGFZ)

### Управление Parser Service

```bash
# Обновить все курсы
update-rates

# Обновить только криптовалюты
update-rates --source coingecko

# Обновить только фиатные валюты
update-rates --source exchangerate

# Посмотреть статус Parser Service
parser-status

# Посмотреть статистику исторических данных
exchange-stats

# Посмотреть историю курсов
view-history --currency BTC --limit 5

# Очистить старые записи истории
cleanup-history --days 30
```

### Тестирование и отладка

```bash
# Тестирование Parser Service
parser-test

# Посмотреть логи
view-logs --lines 10

# Список поддерживаемых валют
list-currencies
```

### Пример выполнения команд проекта (часть 2)

[![asciicast](https://asciinema.org/a/GIsR7FeNBLPeX7H3JXSqCQT6S.svg)](https://asciinema.org/a/GIsR7FeNBLPeX7H3JXSqCQT6S)

## 🔧 Конфигурация

API ключи
Для работы с реальными курсами валют:
1. ExchangeRate-API (фиатные валюты):
- Зарегистрируйтесь на exchangerate-api.com
- Получите API ключ
- Установите переменную окружения:
```bash
export EXCHANGERATE_API_KEY="ваш_ключ"
```
2. CoinGecko API (криптовалюты):
- Работает без ключа (бесплатный тариф)

## 🔧 Настройка в коде

Основные настройки в valutatrade_hub/parser_service/config.py:
```bash
# Списки отслеживаемых валют
FIAT_CURRENCIES = ("EUR", "GBP", "RUB", "JPY", "CHF")
CRYPTO_CURRENCIES = ("BTC", "ETH", "LTC", "XRP", "ADA", "SOL", "DOT")

# Интервал обновления (секунды)
UPDATE_INTERVAL = 300  # 5 минут

# TTL кеша курсов (минуты)
RATES_CACHE_TTL_MINUTES = 5
```

## Задать изначальную валюту (portfolios.json), пример:
```json
[
  {
    "user_id": 1,
    "wallets": {
      "USD": {
        "currency_code": "USD",
        "balance": 10000.0
      }
    }
  }
]
```

## Очистить все данные, пример:
```bash
echo "[]" > data/users.json
echo "[]" > data/portfolios.json
echo "[]" > logs/actions.log
echo "[]" > logs/errors.log
```


## ⚙️ TTL кеша и обновление курсов

- Кеш курсов: Хранится в data/rates.json
- TTL: 5 минут (настраивается в settings.py)
- Автообновление: При запросе курса, если кеш устарел, автоматически запускается обновление
- Ручное обновление: Команда update-rates
- Планировщик: Автоматическое обновление каждые 5 минут (через scheduler.py)

## 📊 Поддерживаемые валюты

1. Криптовалюты (через CoinGecko)
- BTC (Bitcoin)
- ETH (Ethereum)
- LTC (Litecoin)
- XRP (Ripple)
- ADA (Cardano)
- SOL (Solana)
- DOT (Polkadot)

Фиатные валюты (через ExchangeRate-API)
- USD (US Dollar) - базовая валюта
- EUR (Euro)
- GBP (British Pound)
- RUB (Russian Ruble)
- JPY (Japanese Yen)
- CHF (Swiss Franc)

## 🛡️ Безопасность
- Пароли: Хеширование SHA-256 для каждого пользователя
- Данные: JSON файлы с атомарной записью (временный файл → переименование)
- Резервные копии: Автоматическое создание бэкапов перед изменениями
- Логирование: Все операции логируются с контекстом пользователя